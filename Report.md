...
### Samples clustering:
Samples were clustered based on the all the available features. Missing values were filled using the mzMine fill gap option.

A Principal component analysis (PCA) was peformed on all the features across the samples. The PCA revealed a clear separation of the Salvia ans Sclarea species, both on the 1st and 2nd PC. Interestingly, this higher distance between these species does not correlate with phylogenetic analyses of the Salvia species, where Salvia and Sclarea appear closely related.

![PCA](https://user-images.githubusercontent.com/103578179/172426464-65f8f503-32cd-4a2d-a1f1-bfbad107a09b.png)

To compare the clustering of the different species to the phylogenetic studies of the Salvia genus [2], we generated a hierchical clustering. Generated dendrograms show the distance between the different features and the different samples. We see that the heterosphace genus is the one clustering at the smallest distance. Species of the Salvia subgenus also seem to cluster together with the exception of S. interrupta.
Interestingly, we can see that species within the Sclarea subgenus do not cluster and result in the largest distances depicted in the graph.
...
On the  left, the 1<sup>st</sup> subnetwork comprises 2 different annotated compounds, both correspnding to triterpens namely Sumaresinolic acid and Asiatic acid. Although other nodes from the graph were not annotated, the small differences in parent masses could suggest that this network comprises of other triterpens with a minimum of 30 carbon atoms.

At right, the 2<sup>nd</sup>  cluster belongs to sumaresinolic acid. It seems to be interesting because this compound is positionned in the hierarchical clustering at the position of Salvia clustering. The negative point is that in this cluster same compound_name is associated to several nodes. Therefore it is not possible to draw more conclusions. 
...
The analysis of the metabolome was challenging because a few features were annotated (1666 features in total, more or less 430 identified and named based on GNPS database, and more or less 215 features depend on the plant, after duplicates were removed compound name associated). These results already allow to show interesting differences between the reported phylogeny of the Salvia species and their metabolome. However, to go deeper in details and have better comparisons, ideally more samples would need to be collected to reduce sampling bias. Additionnaly, other extraction methods could be tested and optimized, as some metabolites could end up being very good discriminants between Salvia species but might not be optimally extracted with this protocol. Finally, additional replicates would enable more robust statistical analysis and would benefit this study.
